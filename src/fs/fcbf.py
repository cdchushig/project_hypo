...
def entropy(vec, base=2):
    """
    Return the empirical entropy H(X) in the input vector.
    :param vec:
    :param base:
    :return:
    """
...
def fcbf(X, y, threshold):
    ...
    slist[:, 1] = idx
    if threshold < 0:
        threshold = np.median(slist[-1, 0])
        print("Using minimum SU value as default threshold: {0}".format(threshold))
    elif threshold >= 1 or threshold > max(slist[:, 0]):
        print("No relevant features selected for given threshold.")
        print("Please lower the threshold and try again.")
        exit()

    slist = slist[slist[:, 0] > threshold, :]  # desc. ordered per SU[i,c]
...
def fcbf_wrapper(inpath, thresh, delim=',', header=False, classAt=-1):
    ...
    if os.path.exists(inpath):
        try:
            ...
            print
            "Success! Dimensions: {0} x {1}".format(d.shape[0], d.shape[1])
        except Exception:
        ...
        try:
            ...
            try:
                outpath = os.path.split(inpath)[0] + '/features_' + os.path.split(inpath)[1]
                np.savetxt(outpath, sbest, fmt="%0.8f,%d", newline="\n", header='SU, 0-based Feature')
                print("File saved successfully. Path: {0}".format(outpath))
            except Exception:
                print("Error encountered while saving file:")
        except Exception:
            print("Error:")
    else:
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        parser.add_argument('-inpath', metavar='', type=str, dest='inpath', help='Path to input file')
        parser.add_argument('-thresh', metavar='', type=float, dest='thresh', help='SU threshold')
        parser.add_argument('-delim', metavar='', type=str, dest='delim', help='File delimiter', default=',')
        parser.add_argument('-header', metavar='', type=bool, dest='header', help='Contains header?', default=False)
        parser.add_argument('-classAt', metavar='', type=int, dest='classAt', help='Index of class column', default=-1)

        args = parser.parse_args()

        fcbf_wrapper(os.path.abspath(args.inpath), args.thresh, args.delim.decode('string_escape'), args.header, args.classAt)
